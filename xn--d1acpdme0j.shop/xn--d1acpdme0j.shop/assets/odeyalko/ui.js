...
(function () {
  ...
  function moneyNumber(v) {
    ...
    var cleaned = String(v == null ? '' : v).replace(/\s+/g, '').replace(',', '.').replace(/[^\d.-]/g, '');
    var n = Number(cleaned);
    return isFinite(n) ? n : 0;
  }
  function fmtRub(n) { return new Intl.NumberFormat('ru-RU').format(Math.round(moneyNumber(n))) + ' ₽'; }
  ...
  function buildCheckout(cartPage) {
    if (!cartPage || cartPage.getAttribute('data-odk-checkout') === 'ready') return;
    var form = cartPage.querySelector('form.t-form, .t-form');
    if (!form) return;
    cartPage.setAttribute('data-odk-checkout', 'ready');
    ...
    var applied = false;
    function discount() {
      var sub = moneyNumber(window.tcart && window.tcart.prodamount);
    ...
    box.querySelector('#odkOrderBtn').addEventListener('click', function () {
      ...
      var payload = {
        ...
        items: products.map(function (p) {
          var qty = Math.max(1, Math.round(moneyNumber(p.quantity) || 1));
          var price = moneyNumber(p.price);
          return { name: p.name, sku: p.sku || '', qty: qty, price: price, sum: price * qty };
        }),
        page: location.href,
  ...
  function cleanCartArtifacts(cartPage) {
    if (!cartPage) return;
    cartPage.querySelectorAll('.t706__cartpage-info-label').forEach(function (el) {
      el.style.display = 'none';
    });
    cartPage.querySelectorAll('*').forEach(function (el) {
      if (el.children.length === 0 && /^\s*undefined\s*$/i.test(el.textContent || '')) el.style.display = 'none';
    });
  }
